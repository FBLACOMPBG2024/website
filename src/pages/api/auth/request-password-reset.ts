...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        ...
        const links = await client
            .db()
            .collection('links')
            .find({ type: "reset-password", targetEmail: email, createdAt: { $gt: threeMinutesAgo } })
            .toArray();

        // If there are any links that are less than 3 minutes old, do not generate a new link
        if (links.length > 0) {
            return res.status(400).json({ message: 'Too many emails sent recently, Try again later.' });
        }
