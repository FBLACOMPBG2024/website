...
async function editTransaction(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const transaction = await client.db().collection("transactions").findOne({ _id: new ObjectId(_id) });
        if (!transaction) {
        ...
        if (transaction.userId.toString() !== user._id.toString()) {
            return res.status(403).json({ message: 'Forbidden: You do not have permission to edit this transaction' });
        }
        ...
        await client.db().collection("transactions").updateOne({ _id: new ObjectId(_id) }, { $set: updatedTransaction });
